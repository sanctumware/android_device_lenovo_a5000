...
static const CALIBRATION_LAYOUT_STRUCT CalLayoutTbl[MAX_CALIBRATION_LAYOUT_NUM]=
{
	...
	{//CALIBRATION_LAYOUT_SENSOR_OTP
		0x00003205, 0x00000007, CAM_CAL_SINGLE_OTP_DATA,
		{		
			{0x00000000, 0x00000000, 0x00000000, DoCamCalModuleVersion}, //CAMERA_CAM_CAL_DATA_MODULE_VERSION
			...
			{0x00000001, 0x00003204, 0x0000000E, DoCamCal2AGain}, //CAMERA_CAM_CAL_DATA_3A_GAIN
			{0x00000000, 0x00000000, 0x00000000, DoCamCal3DGeo}  //CAMERA_CAM_CAL_DATA_3D_GEO
		}
	}	
};
...
static MINT32 LSCENABLE=1;
static MINT32 AWBENABLE=1;
static MINT32 AFENABLE=1;
    ...
    { 
        ...
        else
        {
            ...
            if(0x1&AWBAFConfig){
                ...
                cam_calCfg.pu1Params = (u8 *)&AWBINFO;            
                ioctlerr= ioctl(CamcamFID, CAM_CALIOC_G_READ, &cam_calCfg);	
				CAM_CAL_LOG_IF(dumpEnable,"Read CalGain OK\n");
                 for(int i=0;i<12;i++)
                 	CAM_CAL_LOG_IF(dumpEnable,"%0x  ",AWBINFO[i]);
				 if(AWBINFO[0]==0xff && AWBINFO[1]==0xff)
				 	ioctlerr=1;
    		if(!ioctlerr)
                {                
    				// Get min gain
    				URG=AWBINFO[0]<<2|((AWBINFO[1]>>6)&0x03);
    				UBG=AWBINFO[2]<<2|((AWBINFO[3]>>6)&0x03);
    				GRG=AWBINFO[6]<<2|((AWBINFO[7]>>6)&0x03);
    				GBG=AWBINFO[8]<<2|((AWBINFO[9]>>6)&0x03);
					
                pCamCalData->Single2A.S2aAwb.rUnitGainu4R = (u32)((512.0/URG)*512);
                pCamCalData->Single2A.S2aAwb.rUnitGainu4G = (u32)( 512);
                pCamCalData->Single2A.S2aAwb.rUnitGainu4B  = (u32)((512.0/UBG)*512);
    
    	        pCamCalData->Single2A.S2aAwb.rGoldGainu4R = (u32)((512.0/GRG)*512);
                pCamCalData->Single2A.S2aAwb.rGoldGainu4G = (u32)( 512);
                pCamCalData->Single2A.S2aAwb.rGoldGainu4B  = (u32)((512.0/GBG)*512);
    			
                ////Only AWB Gain Gathering <////
                #ifdef DEBUG_CALIBRATION_LOAD
                CAM_CAL_LOG_IF(dumpEnable,"\n======================AWB CAM_CAL==================\n");
                CAM_CAL_LOG_IF(dumpEnable,"[rCalGain.u4R] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4R);
                CAM_CAL_LOG_IF(dumpEnable,"[rCalGain.u4G] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4G);
                CAM_CAL_LOG_IF(dumpEnable,"[rCalGain.u4B] = %d\n", pCamCalData->Single2A.S2aAwb.rUnitGainu4B);	
...
static UINT32 DoCamCalLayoutCheck(void)
{
    ...
    if ((gIsInitedCamCal==CAM_CAL_LAYOUT_RTN_QUEUE) && (IMX219_CAM_CALInit() != CAM_CAL_NONE) && (IMX219_CAM_CALDeviceName(&cBuf[0]) == 0))
    {
...
//    CAM_CAL_LOG_IF(dumpEnable,"CAM_CALGetCalData(0x%8x)----\n",(unsigned int)pGetSensorCalData);
	...
	property_get("debug.lsc_otp.enable", value, "1");
    LSCENABLE = atoi(value);
	property_get("debug.awb_otp.enable", value, "1");
    AWBENABLE = atoi(value);
	property_get("debug.af_otp.enable", value, "1");
    AFENABLE = atoi(value);
    //====== Get Property ======
    ...
    {  
        ...
        if ((IMX219_CAM_CALInit() != CAM_CAL_NONE) && (IMX219_CAM_CALDeviceName(&cBuf[0]) == 0))
        {
