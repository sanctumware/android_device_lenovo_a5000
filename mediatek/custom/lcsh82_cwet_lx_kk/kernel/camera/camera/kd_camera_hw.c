...
u32 pinSet[2][8] = {
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX219_MIPI_RAW, currSensorName)))
    {
        ...
        //DVDD
#endif
        if(TRUE != hwPowerOn(CAMERA_SUB_POWER_VCAM_D, VOL_1200,mode_name))
        {
             PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
             //return -EIO;
             //goto _kdCISModulePowerOn_exit_;
	...
	else if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2508_MIPI_RAW,currSensorName)))
	{
       	...
       	//DOVDD
		    if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
		    {
		        PK_DBG("[CAMERA SENSOR OV12830] Fail to enable analog power CAMERA_POWER_VCAM_A\n");
		        //return -EIO;
		        //goto _kdCISModulePowerOn_exit_;
		    }
		    mdelay(10);

		    if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name))// 1500
		    {
		         PK_DBG("[CAMERA SENSOR OV12830] Fail to enable digital power CAMERA_POWER_VCAM_D\n");
		         //return -EIO;
		         //goto _kdCISModulePowerOn_exit_;
		    }
		    mdelay(10);
		    ...
		    if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name)) 
		    {
		        PK_DBG("[CAMERA SENSOR OV12830] Fail to enable analog power CAMERA_POWER_VCAM_A2\n");
		        //return -EIO;
		        goto _kdCISModulePowerOn_exit_;
		    }
			 mdelay(10);
	...
	else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV2680_MIPI_RAW, currSensorName)))
    {
	    ...
	    if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMRST]) {
            if(mt_set_gpio_mode(pinSet[1-pinSetIdx][IDX_PS_CMRST],pinSet[1-pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])){PK_DBG("[CAMERA LENS] set gpio mode failed!! (CMPDN)\n");}
            if(mt_set_gpio_dir(pinSet[1-pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS] set gpio dir failed!! (CMPDN)\n");}
            mdelay(2);
            if(mt_set_gpio_out(pinSet[1-pinSetIdx][IDX_PS_CMRST],pinSet[1-pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! (CMPDN)\n");}
		    mdelay(2);
        }
