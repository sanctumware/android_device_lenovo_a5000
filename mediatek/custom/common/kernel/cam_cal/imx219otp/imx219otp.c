...
extern int get_otp_wb(u8 *buf,u32 length);
extern void IMX219_readpage(kal_uint16 page, BYTE *buf);
...
static int iWriteCAM_CAL(u16 a_u2Addr  , u32 a_u4Bytes, u16 puDataInBytes)
{
    int  i4RetValue = 0;
    //u32 u4Index = 0;
	int retry = 3;
...
static unsigned char get_module_id()
{
	BYTE pModuleInfoData[64] = {0};
      unsigned char temph = 0;
      unsigned char templ = 0;
      kal_uint16 sum;
	  
       IMX219_readpage(0,pModuleInfoData);
	   kal_uint16 nModuleFlag = pModuleInfoData[0];
	if((nModuleFlag & 0x30) == 0x10)
      {
        Module_id=pModuleInfoData[13];
      }
	 else if((nModuleFlag & 0xC0) == 0x40)
	{
		Module_id=pModuleInfoData[1];
	}
	else
		return 1;
	return 0;
...
#if 0
///////add by liangzhiqiang start
unsigned char golden_r_Gr;
unsigned char golden_Gb_Gr;
unsigned char golden_b_Gr;

unsigned char current_r_Gr;
unsigned char current_Gb_Gr;
unsigned char current_b_Gr;
unsigned char Module_id=0;

static void IMX219_readpage(kal_uint16 page, BYTE *buf)
{
   kal_uint16 cnt = 0;
   USHORT i = 0x3204;
   if(page<0 || page>11)
...
static unsigned char get_otp_wb(unsigned char *buf,int length)
{
      //WB flag check
...
//	pOneByteBuff = (u8 *)kmalloc(I2C_UNIT_SIZE, GFP_KERNEL);
  ...
  // *pBuff = current_r_Gr;
  // *(pBuff+1) = current_r_Gr;
  // *(pBuff+2) = current_r_Gr;
 //  *(pBuff+3) = current_r_Gr;
...
static bool isHaveOtpData(u32 offset)
{
    u16 flag = 0;
    if(0x3806 == offset)
    {
...
static void saveOtpData(u32 offset, u16 data)
{
    if(0x3806 == offset)
    {
...
static u16 restoreOtpData(u32 offset)
{
    u16 data = 0;
    ...
    switch(a_u4Command)
    {
		...
		if(ptempbuf->u4Offset==0x3205) 
		{
			...
			if(ret)
				...
				pWorkingBuff[0]=Module_id;
			CAM_CALDB("[IMX219] Module id= 0x%x \n", pWorkingBuff[0]);
		}
		else
		{
		    ret=get_otp_wb(pWorkingBuff,ptempbuf->u4Length);
			if(ret)
			{
				for(i=0;i<(ptempbuf->u4Length);i++)
					pWorkingBuff[i]=0xff;
			}
			if(ptempbuf->u4Length == 12)
			{
			awb_flag = pWorkingBuff[0];
			}
			for(i=0;i<(ptempbuf->u4Length);i++)	
			    CAM_CALDB("[imx219] otp  0x%x \n", pWorkingBuff[i]);
