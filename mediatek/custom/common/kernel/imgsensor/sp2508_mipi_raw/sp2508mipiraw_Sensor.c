...
SP2508MIPI_VIDEO_LINE_LENGTH_PIXELS,SP2508MIPI_VIDEO_FRAME_LENGTH_LINES,SP2508MIPI_FULL_LINE_LENGTH_PIXELS,SP2508MIPI_FULL_FRAME_LENGTH_LINES,320,16,320,16,320,16,30};
...
//#define SP2508MIPI_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, SP2508MIPI_WRITE_ID)
...
void SP2508MIPI_write_shutter(kal_uint16 shutter)
{
	...
	if (SP2508MIPI_sensor.pv_mode == KAL_TRUE) 
	 {
	     max_exp_shutter = SP2508MIPI_PV_FRAME_LENGTH_LINES + SP2508MIPI_sensor.pv_dummy_lines-3;//5//hanlei
        }
	else if (SP2508MIPI_sensor.video_mode== KAL_TRUE) 
       {
            max_exp_shutter = SP2508MIPI_VIDEO_FRAME_LENGTH_LINES + SP2508MIPI_sensor.video_dummy_lines-3;//5//hanlei
       }	 
	else if (SP2508MIPI_sensor.capture_mode== KAL_TRUE) 
	{
           max_exp_shutter = SP2508MIPI_FULL_FRAME_LENGTH_LINES + SP2508MIPI_sensor.cp_dummy_lines-1;//5//hanlei
	}	 
	else
	{	 	
	 ...
	 if (SP2508MIPI_sensor.pv_mode == KAL_TRUE) 
	 {
             frame_length =SP2508MIPI_PV_FRAME_LENGTH_LINES+ SP2508MIPI_sensor.pv_dummy_lines + extra_lines;
             line_length = SP2508MIPI_PV_LINE_LENGTH_PIXELS+ SP2508MIPI_sensor.pv_dummy_pixels;
             spin_lock_irqsave(&sp2508_drv_lock,flags);
             SP2508MIPI_sensor.pv_line_length = line_length;
             SP2508MIPI_sensor.pv_frame_length = frame_length;
             spin_unlock_irqrestore(&sp2508_drv_lock,flags);
	 }
	 else if (SP2508MIPI_sensor.video_mode== KAL_TRUE) 
	 {
             frame_length = SP2508MIPI_VIDEO_FRAME_LENGTH_LINES+ SP2508MIPI_sensor.video_dummy_lines + extra_lines;
             line_length =SP2508MIPI_VIDEO_LINE_LENGTH_PIXELS + SP2508MIPI_sensor.video_dummy_pixels;
             spin_lock_irqsave(&sp2508_drv_lock,flags);
             SP2508MIPI_sensor.video_line_length = line_length;
             SP2508MIPI_sensor.video_frame_length = frame_length;
             spin_unlock_irqrestore(&sp2508_drv_lock,flags);
	 } 
	 else if(SP2508MIPI_sensor.capture_mode== KAL_TRUE)
	 {
             frame_length = SP2508MIPI_FULL_FRAME_LENGTH_LINES+ SP2508MIPI_sensor.cp_dummy_lines + extra_lines;
             line_length =SP2508MIPI_FULL_LINE_LENGTH_PIXELS + SP2508MIPI_sensor.cp_dummy_pixels;
             spin_lock_irqsave(&sp2508_drv_lock,flags);
             SP2508MIPI_sensor.cp_line_length = line_length;
             SP2508MIPI_sensor.cp_frame_length = frame_length;
             spin_unlock_irqrestore(&sp2508_drv_lock,flags);
	 }
	 else
	 { 	
  ...
  /*   if(shutter < SP2508MIPI_sensor.pv_pclk/(100*(SP2508MIPI_sensor.pv_line_length+SP2508MIPI_sensor.pv_dummy_pixels) ))
	 	shutter = SP2508MIPI_sensor.pv_pclk/(100*(SP2508MIPI_sensor.pv_line_length + SP2508MIPI_sensor.pv_dummy_pixels));*/
    ...
    /*wm    if(iGain >= BASEGAIN && iGain <= 16 * BASEGAIN)
       {
               ...
               iRegGain = SP2508MIPIGain2Reg(iGain);

               if (iRegGain < 0x10) iRegGain = 0x10;
               {
   	              SP2508MIPI_write_cmos_sensor(0xfd, 0x01);
   	              SP2508MIPI_write_cmos_sensor(0x24, iRegGain);
   	              SP2508MIPI_write_cmos_sensor(0x01, 0x01);
		       SENSORDB("gjh SP2508MIPI_SetGain = %d",iRegGain);
                }
       } 
       else 
	{
   	     ...
   	     case CMMCLK_CURRENT:
   	    	   switch (item_idx)
   	    	   {
                 	  case 0:
                   	      ...
                   	      //temp_reg=SP2508MIPISensorReg[CMMCLK_CURRENT_INDEX].Para;
            ...
            switch (item_idx)
            {
                case 0:
                    ...
                    {			
                        ...
                        spin_unlock(&sp2508_drv_lock);
                        //SP2508MIPI_set_isp_driving_current(ISP_DRIVING_2MA);
                    }
                    else if(ItemValue==3 || ItemValue==4)
                    {
                        ...
                        spin_unlock(&sp2508_drv_lock);
                        //SP2508MIPI_set_isp_driving_current(ISP_DRIVING_4MA);
                    }
                    else if(ItemValue==5 || ItemValue==6)
                    {
                        ...
                        spin_unlock(&sp2508_drv_lock);
                        //SP2508MIPI_set_isp_driving_current(ISP_DRIVING_6MA);
                    }
                    else
                    {
                    	    spin_lock(&sp2508_drv_lock);    
                        SP2508MIPISensorReg[CMMCLK_CURRENT_INDEX].Para = ISP_DRIVING_8MA;
                    	    spin_unlock(&sp2508_drv_lock);
                        //SP2508MIPI_set_isp_driving_current(ISP_DRIVING_8MA);
                    }
                    break;
                default:
                    ASSERT(0);
...
static void SP2508MIPI_SetDummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
    ...
    if(SP2508MIPI_sensor.pv_mode == KAL_TRUE)
    {
	 ...
	 //copy hanlei
	 SP2508MIPI_write_cmos_sensor(0xfd, 0x01);
	 SP2508MIPI_write_cmos_sensor(0x05, (SP2508MIPI_sensor.pv_dummy_lines>>8) & 0xFF);
	 SP2508MIPI_write_cmos_sensor(0x06, SP2508MIPI_sensor.pv_dummy_lines & 0xFF);	
//	 SP2508MIPI_write_cmos_sensor(0x07, (SP2508MIPI_sensor.pv_dummy_lines >>8) & 0xFF);
//	 SP2508MIPI_write_cmos_sensor(0x08, SP2508MIPI_sensor.pv_dummy_lines & 0xFF);	
	 SP2508MIPI_write_cmos_sensor(0x09, (SP2508MIPI_sensor.pv_dummy_pixels >>8) & 0x0F);
	 SP2508MIPI_write_cmos_sensor(0x0A, SP2508MIPI_sensor.pv_dummy_pixels & 0xFF);	
	 SP2508MIPI_write_cmos_sensor(0x01, 0x01);	
    }
    else if(SP2508MIPI_sensor.video_mode== KAL_TRUE)
    {
	 ...
	 //copy hanlei
	 SP2508MIPI_write_cmos_sensor(0xfd, 0x01);
	 SP2508MIPI_write_cmos_sensor(0x05, (SP2508MIPI_sensor.video_dummy_lines>>8) & 0xFF);
	 SP2508MIPI_write_cmos_sensor(0x06, SP2508MIPI_sensor.video_dummy_lines & 0xFF);	
//	 SP2508MIPI_write_cmos_sensor(0x07, (SP2508MIPI_sensor.video_dummy_lines >>8) & 0xFF);
//	 SP2508MIPI_write_cmos_sensor(0x08, SP2508MIPI_sensor.video_dummy_lines & 0xFF);	
	 SP2508MIPI_write_cmos_sensor(0x09, (SP2508MIPI_sensor.video_dummy_pixels >>8) & 0x0F);
	 SP2508MIPI_write_cmos_sensor(0x0A, SP2508MIPI_sensor.video_dummy_pixels & 0xFF);	 
	 SP2508MIPI_write_cmos_sensor(0x01, 0x01);
    }
    else if(SP2508MIPI_sensor.capture_mode== KAL_TRUE) 
    {
	 ...
	 //copy hanlei
	 SP2508MIPI_write_cmos_sensor(0xfd, 0x01);
	 SP2508MIPI_write_cmos_sensor(0x05, (SP2508MIPI_sensor.cp_dummy_lines>>8) & 0xFF);
	 SP2508MIPI_write_cmos_sensor(0x06, SP2508MIPI_sensor.cp_dummy_lines & 0xFF);	
	// SP2508MIPI_write_cmos_sensor(0x07, (SP2508MIPI_sensor.cp_dummy_lines >>8) & 0xFF);
	// SP2508MIPI_write_cmos_sensor(0x08, SP2508MIPI_sensor.cp_dummy_lines & 0xFF);	
	 SP2508MIPI_write_cmos_sensor(0x09, (SP2508MIPI_sensor.cp_dummy_pixels >>8) & 0x0F);
	 SP2508MIPI_write_cmos_sensor(0x0A, SP2508MIPI_sensor.cp_dummy_pixels & 0xFF);	
	 SP2508MIPI_write_cmos_sensor(0x01, 0x01);
    }
    else
    {
...
static void SP2508MIPI_Sensor_Init(void)
{
  ...
  SP2508MIPI_write_cmos_sensor(0x2f,0x08); //0x10 pll clk 84M//0x08 pll clk 60M
  SP2508MIPI_write_cmos_sensor(0x1c,0x03); //pull down parallel pad
  SP2508MIPI_write_cmos_sensor(0xfd,0x01);

  SP2508MIPI_write_cmos_sensor(0x21,0xee);//3base fliker gpw20140607
  
  SP2508MIPI_write_cmos_sensor(0x03,0x01); //exp time, 1 base
  SP2508MIPI_write_cmos_sensor(0x04,0x6b);
  SP2508MIPI_write_cmos_sensor(0x06,0x10); //vblank
  SP2508MIPI_write_cmos_sensor(0x24,0xf0); //pga gain 15x
  SP2508MIPI_write_cmos_sensor(0x01,0x01); //enable reg write
  SP2508MIPI_write_cmos_sensor(0x2b,0xc6); //readout vref
  ...
  SP2508MIPI_write_cmos_sensor(0x8a,0x33); //pixel bias 2uA //pixel 1ua //20140607 0x33
  SP2508MIPI_write_cmos_sensor(0x8b,0x33); 
  SP2508MIPI_write_cmos_sensor(0x19,0xf3); //icom1 1.7u, icom2 0.6u 
  SP2508MIPI_write_cmos_sensor(0x11,0x30); //rst num
  SP2508MIPI_write_cmos_sensor(0xd0,0x01); //boost2 enable
  SP2508MIPI_write_cmos_sensor(0xd1,0x01); //boost2 start point h'1do
  SP2508MIPI_write_cmos_sensor(0xd2,0xd0);  
  SP2508MIPI_write_cmos_sensor(0x55,0x10);
  SP2508MIPI_write_cmos_sensor(0x58,0x40);//0x30 //20140607
  ...
  SP2508MIPI_write_cmos_sensor(0x00,0x8c);
  SP2508MIPI_write_cmos_sensor(0x01,0x8c);
  SP2508MIPI_write_cmos_sensor(0x03,0x8c);
  SP2508MIPI_write_cmos_sensor(0x04,0x8c);  
...
SP2508MIPI_write_cmos_sensor(0xfd,0x00);
SP2508MIPI_write_cmos_sensor(0xfd,0x00);
//SP2508MIPI_write_cmos_sensor(0x34,0x02);//gpw
SP2508MIPI_write_cmos_sensor(0x35,0x20);
SP2508MIPI_write_cmos_sensor(0x2f,0x10);
SP2508MIPI_write_cmos_sensor(0x1b,0x1f);
SP2508MIPI_write_cmos_sensor(0x1c,0x03);
SP2508MIPI_write_cmos_sensor(0xfd,0x01);
SP2508MIPI_write_cmos_sensor(0x03,0x01);
SP2508MIPI_write_cmos_sensor(0x04,0x6b);
SP2508MIPI_write_cmos_sensor(0x06,0x10);
SP2508MIPI_write_cmos_sensor(0x31,0x00);
SP2508MIPI_write_cmos_sensor(0x24,0x40);
SP2508MIPI_write_cmos_sensor(0x01,0x01);
SP2508MIPI_write_cmos_sensor(0x2b,0xc6);//gpw c7
SP2508MIPI_write_cmos_sensor(0x2c,0xf0);
SP2508MIPI_write_cmos_sensor(0x2e,0x20);
SP2508MIPI_write_cmos_sensor(0x79,0x42);
SP2508MIPI_write_cmos_sensor(0x85,0x0f);
SP2508MIPI_write_cmos_sensor(0x09,0x02);//01 gpw
SP2508MIPI_write_cmos_sensor(0x0a,0x00);//40 gpw
//SP2508MIPI_write_cmos_sensor(0x3c,0x02);

SP2508MIPI_write_cmos_sensor(0x25,0xf2);
SP2508MIPI_write_cmos_sensor(0x26,0x10);
SP2508MIPI_write_cmos_sensor(0x2a,0xea);//gpw ba//ea//2014/5/20
SP2508MIPI_write_cmos_sensor(0x8a,0x55);
SP2508MIPI_write_cmos_sensor(0x8b,0x55);
SP2508MIPI_write_cmos_sensor(0x19,0xf3);
//SP2508MIPI_write_cmos_sensor(0x3c,0x02); //03 gpw
//SP2508MIPI_write_cmos_sensor(0xd0,0x00); //03 gpw
SP2508MIPI_write_cmos_sensor(0x0e,0x01);
SP2508MIPI_write_cmos_sensor(0x0f,0x01);
SP2508MIPI_write_cmos_sensor(0x11,0x30);
SP2508MIPI_write_cmos_sensor(0x55,0x10);
...
SP2508MIPI_write_cmos_sensor(0x91,0xc0);
SP2508MIPI_write_cmos_sensor(0x92,0x01);
SP2508MIPI_write_cmos_sensor(0xa1,0x02);
SP2508MIPI_write_cmos_sensor(0xaa,0x00);
SP2508MIPI_write_cmos_sensor(0xfd,0x01);
SP2508MIPI_write_cmos_sensor(0xac,0x01);
SP2508MIPI_write_cmos_sensor(0xfb,0x25);//gpw 05
SP2508MIPI_write_cmos_sensor(0xff,0xff);
	...
	SP2508MIPI_write_cmos_sensor(0x10,0x04);
	SP2508MIPI_write_cmos_sensor(0x11,0x10);
	SP2508MIPI_write_cmos_sensor(0x12,0x04);
	SP2508MIPI_write_cmos_sensor(0x13,0x01);
	SP2508MIPI_write_cmos_sensor(0x15,0x00);
	SP2508MIPI_write_cmos_sensor(0x21,0x03);
	SP2508MIPI_write_cmos_sensor(0x20,0x3f);
	SP2508MIPI_write_cmos_sensor(0x0e,0x00);
	SP2508MIPI_write_cmos_sensor(0x26,0x00);

	SP2508MIPI_write_cmos_sensor(0xfd,0x05);
	SP2508MIPI_write_cmos_sensor(0x00,0xc1);
	SP2508MIPI_write_cmos_sensor(0x02,0x10);
	SP2508MIPI_write_cmos_sensor(0x03,0x05);
	SP2508MIPI_write_cmos_sensor(0x04,0xcc);
	SP2508MIPI_write_cmos_sensor(0x05,0x03);
	SP2508MIPI_write_cmos_sensor(0xfd,0x06);
	SP2508MIPI_write_cmos_sensor(0xfb,0x00);
	SP2508MIPI_write_cmos_sensor(0xc6,0x80);
	SP2508MIPI_write_cmos_sensor(0xc7,0x80);
	SP2508MIPI_write_cmos_sensor(0xc8,0x80);
	SP2508MIPI_write_cmos_sensor(0xc9,0x80);
	
	SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	SP2508MIPI_write_cmos_sensor(0x09,0x00);//14/04/10
	SP2508MIPI_write_cmos_sensor(0x0a,0x83);//14/04/10
	SP2508MIPI_write_cmos_sensor(0x0b,0x02);//14/04/10
	SP2508MIPI_write_cmos_sensor(0x0c,0x33);
	SP2508MIPI_write_cmos_sensor(0x0d,0x33);
	SP2508MIPI_write_cmos_sensor(0x21,0x1f);
	SP2508MIPI_write_cmos_sensor(0x29,0x03);
	SP2508MIPI_write_cmos_sensor(0x2a,0xff);
	SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	SP2508MIPI_write_cmos_sensor(0x0f,0x7f);
	SP2508MIPI_write_cmos_sensor(0x10,0x7e);
	SP2508MIPI_write_cmos_sensor(0x11,0x00);
	SP2508MIPI_write_cmos_sensor(0x12,0x00);
	SP2508MIPI_write_cmos_sensor(0x13,0x2f);
	SP2508MIPI_write_cmos_sensor(0x16,0x00);
	SP2508MIPI_write_cmos_sensor(0x17,0x2f);
	SP2508MIPI_write_cmos_sensor(0x14,0x00);
	SP2508MIPI_write_cmos_sensor(0x15,0x38);
	SP2508MIPI_write_cmos_sensor(0x6b,0x32);
	SP2508MIPI_write_cmos_sensor(0x6c,0x32);
	SP2508MIPI_write_cmos_sensor(0x6f,0x32);
	SP2508MIPI_write_cmos_sensor(0x70,0x40);
	SP2508MIPI_write_cmos_sensor(0x71,0x40);
	SP2508MIPI_write_cmos_sensor(0x72,0x40);
	SP2508MIPI_write_cmos_sensor(0x73,0x33);
	SP2508MIPI_write_cmos_sensor(0x74,0x38);
	SP2508MIPI_write_cmos_sensor(0x75,0x38);
	SP2508MIPI_write_cmos_sensor(0x76,0x44);
	SP2508MIPI_write_cmos_sensor(0x77,0x38);
	SP2508MIPI_write_cmos_sensor(0x7a,0x40);
	SP2508MIPI_write_cmos_sensor(0x7e,0x40);
	SP2508MIPI_write_cmos_sensor(0x84,0x40);
	SP2508MIPI_write_cmos_sensor(0x7d,0x0f);

	SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	SP2508MIPI_write_cmos_sensor(0x19,0x03);//0x13guojinhui
	SP2508MIPI_write_cmos_sensor(0x31,0x04);
	SP2508MIPI_write_cmos_sensor(0x24,0x80);
	SP2508MIPI_write_cmos_sensor(0x03,0x0e);
	SP2508MIPI_write_cmos_sensor(0x04,0xec);
	SP2508MIPI_write_cmos_sensor(0x22,0x1a);
	SP2508MIPI_write_cmos_sensor(0x2e,0x08);
	SP2508MIPI_write_cmos_sensor(0x1e,0x01);
	SP2508MIPI_write_cmos_sensor(0x1d,0x04);
	SP2508MIPI_write_cmos_sensor(0x20,0xdf);
	SP2508MIPI_write_cmos_sensor(0x25,0x8d);
	SP2508MIPI_write_cmos_sensor(0x27,0xa9);//0xb1guojinhui
	SP2508MIPI_write_cmos_sensor(0x1a,0x43);
	SP2508MIPI_write_cmos_sensor(0x28,0x00);
	SP2508MIPI_write_cmos_sensor(0x40,0x05);
	SP2508MIPI_write_cmos_sensor(0x3f,0x00);
	SP2508MIPI_write_cmos_sensor(0x01,0x01);
	
	SP2508MIPI_write_cmos_sensor(0xfd,0x02);
	SP2508MIPI_write_cmos_sensor(0x37,0x00);
	SP2508MIPI_write_cmos_sensor(0x38,0x04);
	SP2508MIPI_write_cmos_sensor(0x3b,0x00);
	SP2508MIPI_write_cmos_sensor(0x3c,0x02);
	SP2508MIPI_write_cmos_sensor(0x39,0x03);
	SP2508MIPI_write_cmos_sensor(0x3a,0xcc);
	SP2508MIPI_write_cmos_sensor(0x3d,0x02);
	SP2508MIPI_write_cmos_sensor(0x3e,0x88);
	SP2508MIPI_write_cmos_sensor(0x33,0x0f);
	SP2508MIPI_write_cmos_sensor(0x3f,0x01);
	SP2508MIPI_write_cmos_sensor(0xfd,0x03);
	SP2508MIPI_write_cmos_sensor(0xaa,0x46);
	SP2508MIPI_write_cmos_sensor(0xab,0x01);
	SP2508MIPI_write_cmos_sensor(0xac,0xf5);
	SP2508MIPI_write_cmos_sensor(0xad,0x00);
	//SP2508MIPI_write_cmos_sensor(0xff,0xff);
	#endif
...
static void PreviewSetting(void)
{
	...
	//1296x972
	SP2508MIPI_write_cmos_sensor(0xfd,0x00);
	SP2508MIPI_write_cmos_sensor(0x10,0x04);//48MHz Pclk//max 11.9820fps//one  line time:41.6666us
	SP2508MIPI_write_cmos_sensor(0x11,0x10);

	SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	SP2508MIPI_write_cmos_sensor(0x19,0x03);
	SP2508MIPI_write_cmos_sensor(0x31,0x04);
	SP2508MIPI_write_cmos_sensor(0x01,0x01);
...
{	//77 capture setting
	...
	//2592x1944
	SP2508MIPI_write_cmos_sensor(0xfd,0x00);
	SP2508MIPI_write_cmos_sensor(0x10,0x04);//hanlei
	SP2508MIPI_write_cmos_sensor(0x11,0x00);

	//SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	//SP2508MIPI_write_cmos_sensor(0x09,0x01);
	//SP2508MIPI_write_cmos_sensor(0x0a,0x43);//50us max9.99fps

	SP2508MIPI_write_cmos_sensor(0xfd,0x01);
	SP2508MIPI_write_cmos_sensor(0x19,0x00);
	SP2508MIPI_write_cmos_sensor(0x31,0x00);
	SP2508MIPI_write_cmos_sensor(0x01,0x01);

	SP2508MIPI_write_cmos_sensor(0xfd,0x02);
	SP2508MIPI_write_cmos_sensor(0x37,0x00);
	SP2508MIPI_write_cmos_sensor(0x38,0x08);
	SP2508MIPI_write_cmos_sensor(0x3b,0x00);
	SP2508MIPI_write_cmos_sensor(0x3c,0x04);
	SP2508MIPI_write_cmos_sensor(0x39,0x07);
	SP2508MIPI_write_cmos_sensor(0x3a,0x98);
	SP2508MIPI_write_cmos_sensor(0x3d,0x05);
	SP2508MIPI_write_cmos_sensor(0x3e,0x10);

	SP2508MIPI_write_cmos_sensor(0xfd,0x05);
	SP2508MIPI_write_cmos_sensor(0x02,0x20);
	SP2508MIPI_write_cmos_sensor(0x03,0x0a);
	SP2508MIPI_write_cmos_sensor(0x04,0x98);
	SP2508MIPI_write_cmos_sensor(0x05,0x07);
...
UINT32 SP2508MIPIOpen(void)
{
    ...
    //  #endif
	//  sensorid = SP2508MIPI_read_cmos_sensor(0x02);
...
*
*************************************************************************/
static u8 sp2508_rc = 0;
static u8 sp2508_used = 0;
...
//end test
    ...
    if (*sensorID == SP2508MIPI_SENSOR_ID) {
				...
				dev->device_type = "CCM";
				dev->device_module = "shanglaishi";
				dev->device_vendor	 ="shanglaishi"; 	  
				dev->device_ic	 = "SP2508";
				dev->device_version = "CDGM+unknown";
				dev->device_info = "200W";
...
{
    //SP2508MIPI_write_cmos_sensor(0x0100,0x00);
    return ERROR_NONE;
}	/* SP2508MIPIClose() */
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	SP2508MIPI_Auto_Flicker_mode = KAL_FALSE;    
     ...
     /*  if((image_window->ExposureWindowWidth<=SP2508MIPI_REAL_PV_WIDTH)&&(image_window->ExposureWindowHeight<=SP2508MIPI_REAL_PV_HEIGHT))
	{		
		spin_unlock(&sp2508_drv_lock);
		PreviewSetting();
		...
		SP2508MIPI_sensor.cp_line_length=SP2508MIPI_PV_LINE_LENGTH_PIXELS+SP2508MIPI_sensor.pv_dummy_pixels;
		SP2508MIPI_sensor.cp_frame_length=SP2508MIPI_PV_FRAME_LENGTH_LINES	+SP2508MIPI_sensor.pv_dummy_lines;
		spin_unlock(&sp2508_drv_lock);
		iStartX = SP2508MIPI_IMAGE_SENSOR_PV_STARTX;
		iStartY = SP2508MIPI_IMAGE_SENSOR_PV_STARTY;
		image_window->GrabStartX=iStartX;
		image_window->GrabStartY=iStartY;
		image_window->ExposureWindowWidth=SP2508MIPI_REAL_PV_WIDTH ;
		image_window->ExposureWindowHeight=SP2508MIPI_REAL_PV_HEIGHT;
		SP2508MIPI_SetDummy(SP2508MIPI_sensor.pv_dummy_pixels, SP2508MIPI_sensor.pv_dummy_lines);   
	}
...
{ 
	...
	/*  wm  switch(ScenarioId){
			...
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG://hhl 2-28
				pSensorInfo->SensorFullResolutionX=SP2508MIPI_REAL_CAP_WIDTH;
				pSensorInfo->SensorFullResolutionY=SP2508MIPI_REAL_CAP_HEIGHT;
			...
			case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				pSensorInfo->SensorPreviewResolutionX=SP2508MIPI_REAL_VIDEO_WIDTH;
				pSensorInfo->SensorPreviewResolutionY=SP2508MIPI_REAL_VIDEO_HEIGHT;
				...
				SENSORDB("MSDK_SCENARIO_ID_VIDEO_PREVIEW:pSensorInfo->SensorCameraPreviewFrameRate =%d", pSensorInfo->SensorCameraPreviewFrameRate);
			       break;
			default:
        			pSensorInfo->SensorPreviewResolutionX=SP2508MIPI_REAL_PV_WIDTH;
        			pSensorInfo->SensorPreviewResolutionY=SP2508MIPI_REAL_PV_HEIGHT;
				pSensorInfo->SensorCameraPreviewFrameRate=30;//15;//30//hanlei
...
UINT32 SP2508MIPISetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
	...
	{   // enable auto flicker   
		...
		{ // in the video mode, reset the frame rate
           ...
           // SP2508MIPI_write_cmos_sensor(0x0104, 1);        
            //SP2508MIPI_write_cmos_sensor(0x0340, (pv_max_frame_rate_lines >>8) & 0xFF);
            //SP2508MIPI_write_cmos_sensor(0x0341, pv_max_frame_rate_lines & 0xFF);	
            //SP2508MIPI_write_cmos_sensor(0x0104, 0);        	
        }
    } 
	else 
	{
		...
		{    // in the video mode, restore the frame rate
            //SP2508MIPI_write_cmos_sensor(0x0104, 1);        
            //SP2508MIPI_write_cmos_sensor(0x0340, (SP2508MIPI_MAX_EXPOSURE_LINES >>8) & 0xFF);
            //SP2508MIPI_write_cmos_sensor(0x0341, SP2508MIPI_MAX_EXPOSURE_LINES & 0xFF);	
           // SP2508MIPI_write_cmos_sensor(0x0104, 0);        	
        }
...
UINT32 SP2508MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	...
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			//lineLength = SP2508MIPI_PV_LINE_LENGTH_PIXELS;
			...
			spin_unlock(&sp2508_drv_lock);
			//SP2508MIPI_SetDummy(0, dummyLine);			
			break;			
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			pclk = SP2508MIPI_sensor.video_pclk;//24000000;//hanlei
			//lineLength = SP2508MIPI_VIDEO_LINE_LENGTH_PIXELS;
...
UINT32 SP2508MIPISetTestPatternMode(kal_bool bEnable)
{
    ...
    if(bEnable) {   // enable color bar   
        //SP2508MIPI_write_cmos_sensor(0x30D8, 0x10);  // color bar test pattern
        //SP2508MIPI_write_cmos_sensor(0x0600, 0x00);  // color bar test pattern
        //SP2508MIPI_write_cmos_sensor(0x0601, 0x02);  // color bar test pattern 
    } else {
        //SP2508MIPI_write_cmos_sensor(0x30D8, 0x00);  // disable color bar test pattern
    }
    return ERROR_NONE;
}
