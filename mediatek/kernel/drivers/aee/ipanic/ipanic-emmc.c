...
static struct aee_oops *emmc_ipanic_oops_copy(void)
{
	...
	xlog_printk(ANDROID_LOG_DEBUG, IPANIC_LOG_TAG, "ipanic_oops_copy OK\n");
	kfree(hdr);
	return oops;
}
...
static void ipanic_write_mmprofile(int offset, struct ipanic_header *hdr)
{
  ...
  if (mmprofile_dump_size == 0 || mmprofile_dump_size > IPANIC_OOPS_MMPROFILE_LENGTH_LIMIT) {
    xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG, "INVALID MMProfile dump size 0x%x", mmprofile_dump_size);
    return;
  }
  ...
  do {
    ...
    if (rc < 0) {
      xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG, "Error writing MMProfile to emmc! (%d)\n", rc);
