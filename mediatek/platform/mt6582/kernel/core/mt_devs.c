...
resource_size_t tee_reserved_size = 0x0;       // kernel_mem_sz is inited in mt_fixup
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
	...
	} else if (tags->hdr.tag == ATAG_TEE_DATA) {
		tee_reserved_size = tags->u.tee_data.tee_size;
	}
    }
    ...
    */
    ...
    if (tee_reserved_size) {
        MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
                "[PHY layout]tee:   (0x%llx)\n",
                (unsigned long long)tee_reserved_size);
    }
...
static struct platform_device actuator_dev_dw9718 = {
	.name		  = "lens_actuator_dw9718",
	.id		  = -1,
};
/*=======================================================================*/
/* MT6575 jogball                                                        */
...
#if 1  //defined(CONFIG_ACTUATOR)
    retval = platform_device_register(&actuator_dev_dw9718);
    if (retval != 0){
        return retval;
    }
#endif
...
unsigned int mtk_get_max_DRAM_size(void)
{
        return kernel_mem_sz + RESERVED_MEM_MODEM + tee_reserved_size;
}
